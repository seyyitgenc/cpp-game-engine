...
public:
    ...
    template <typename... Args>
    static void writeFormatted(int nLevel, const char* fmt, Args... args)
    {
        checkInit();
        if (nLevel >= _nLevel) {
            auto now = std::chrono::system_clock::now();
            ...
            char text[256];
            snprintf(text, 256, fmt, args...);
            std::cout << "[" << std::put_time(local_time, "%H:%M:%S") << "] " << std::string(text) << std::endl;
        }
    }
